...
- встроенные записи каталога имеют `updatedAt = 0`: при подключении к Диску с непустой базой они не дублируются.
  Нетронутая встроенная строка базы (`updatedAt = 0`) при чтении берёт название, группу, интервалы и скрытость из
  текущего кода (`withBuiltinDefaults`): переименования каталога доходят до уже засеянных баз без записи в базу.
...
| Маршрут | Экран |
|---|---|
| `#/` | Главная: переключатель машин, карточка (фото, номер, пробег, чертёж машины — §7.0), «Скоро» (3 ближайших), быстрые кнопки Заправка/ТО/Расход/Пробег, сводка месяца (потрачено, л/100 км) → статистика, последние 5 записей |
...
| `#/more` | Меню «Ещё» |
| `#/stats` | Статистика, вкладки (запоминаются): **Расходы** — стоимость владения, цена км, расход топлива, на что уходят деньги, по годам; **Сервис** — работы и запчасти, средний чек, «Сервис по месяцам» (столбцы/линия), где обслуживаюсь (СТО, «делал сам»), мастера; **Запчасти** — свои и сервиса, на что уходит по узлам (замены, пробег между заменами, бренды), бренды, где покупал. У рейтингов — вид «Список · Столбцы · Доли» (запоминается); строки ведут на место, мастера, историю узла |
| `#/garage`, `#/vehicle/new`, `#/vehicle/:id`, `#/vehicle/:id/edit` | Гараж, архив, карточка и форма машины с VIN-декодером; марка → модель → год → поколение → кузов — из справочника машин (`domain/carCatalog.json`: 80 марок, ~850 моделей, ~2 500 поколений с рестайлингами и годами, русские написания для поиска; грузится отдельным куском), поколение, в которое попадает год, — первым; вписать можно любое |
| `#/documents`, `#/documents/:id` | Документы со сроками и вложениями |
| `#/tires`, `#/tires/:id` | Комплекты шин: бренд, модель и размер — из подсказок (`domain/tireCatalog.ts`: ~115 популярных размеров, ~35 брендов и их модели с сезоном и шипами, прежние имена Nokian → Ikon); можно вписать любое, размер приводится к виду «205/55 R16» |
| `#/places`, `#/places/:id`, `#/masters/:id` | Места и мастера: визиты, всего потрачено, средний чек |
| `#/places/directory` | Справочник СТО и АЗС выбранного города (Ростов-на-Дону — ~1 000 автосервисов и шиномонтажей и ~140 заправок, Евпатория — ~160 и 24; из Яндекс Карт, `domain/directory/<город>.json`, грузится отдельным куском): поиск по названию и улице, вид «Все · СТО · Шины · АЗС», карточка — адрес (ссылка на организацию в Яндекс Картах), телефоны, «Добавить в мои места»; заправка сети называется сетью. Город — на устройстве (localStorage), выбирается здесь и в настройках; не выбран — справочника нигде нет. Поле «Место» у ТО и шиномонтажа и поле «АЗС» с двух букв подсказывают до 8 мест справочника своего вида («название, адрес · Яндекс Карты»), уже свои — нет; выбор заводит своё место с адресом, первым телефоном и ссылкой |
| `#/catalog` | Каталог узлов: свои позиции, скрытие встроенных, интервалы по умолчанию |
| `#/settings`, `#/settings/sync`, `#/settings/data` | Тема, город справочника СТО, синхронизация, выгрузки, о приложении |
| `#/onboarding` | Первый запуск: добавить машину (VIN → поля) → подключить Диск (можно позже) |
| `#/showcase` | Витрина дизайн-системы — только при разработке (`npm run dev`); в сборке для людей нет ни маршрута, ни ссылок |

### 7.0 Чертёж машины на главной (`ui/VehicleSchematic`)

Для машин, у которых есть чертёж, карточка на главной показывает его под именем: 3/4 спереди слева, перед смотрит
влево, линии красятся токенами темы (`--schematic-*`, маска `mask-image`). Чертежи: Skoda Octavia A5 (2004–2013, не
универсал, не Tour), Kia cee'd первого поколения (2006–2012, универсал), Daewoo Lanos (и Chevrolet/ЗАЗ Lanos, Sens,
Chance), BMW X5 F15 (2013–2018); год, поколение и кузов проверяются, только если указаны. Остальные машины — карточка
как раньше.

- **Выбор картинки.** Значок «Картинка машины» справа от имени на карточке (главная и страница машины) открывает
  шторку с плитками: «Автоматически» (что подобралось по марке и модели), все картинки и «Без картинки»; касание
  сразу сохраняет. Машине без подобранной картинки главная показывает кнопку «Выбрать картинку машины» (после
  «Без картинки» — нет). Те же плитки — в форме машины («Картинка на главной»). Выбор хранится в `Vehicle.schematic`
  (id картинки или 'none'; нет поля — подбор), так седану без своей картинки можно поставить Lanos, кроссоверу — X5.

- **Зоны.** Двигатель (масло, фильтры, свечи, ГУР), ГРМ и ремни (ремень/цепь, приводной ремень, помпа) — дальний
  край моторного отсека, тормоза (тормозная жидкость — бачок у стекла, «рентген» трубок к колёсам), охлаждение
  (антифриз — решётка), электрика (аккумулятор), свет, трансмиссия, салон (салонный фильтр, кондиционер, щётки,
  омывайка), переднее колесо (передние колодки и диски, подвеска, рулевое, шины), заднее колесо (задние колодки и
  диски, вся задняя подвеска и стояночный тормоз), свет (лампы и фары). Радиатор, его вентилятор, патрубки и термостат —
  охлаждение; главный цилиндр, усилитель, шланги и трубки — тормоза. Свои узлы — по группе каталога; кузов (кроме
  стекла и трапеции дворников — салон) и «Прочее» на чертеже не отмечаются.
- **Что видно.** Точки — только у зон со «скоро» и «просрочено», цвет по худшему; у точки — «рентген» узла
  (моторный отсек, шкивы ремня, тормозные трубки, колесо, аккумулятор). Выноски — у двух самых срочных зон
  (порядок как в «Скоро»): короткое название и ближайший срок по км или по времени, «ещё N» — другие узлы зоны.
  Легенда: «Просрочено · Скоро · В порядке» по всем напоминаниям машины. Нажатие — `#/reminders`.
- **Новая модель.** Картинка-чертёж из генератора (белые линии на синем, 3/4 спереди слева, без надписей) →
  `tools/schematic-mask.py` → `art/<модель>.webp` + координаты зон в `models.ts` + правило в
  `features/home/schematic.ts`. Все зоны разом показывает витрина (`VehicleSchematic`).
...
- **ТО/ремонт:** форма короткая — название (подсказки «ТО-N», из прошлых), тип, стоимость, дата, пробег, место
  (комбобокс с созданием нового), комплекты шин у переобувки, фото; этого хватает, чтобы внести ремонт одной суммой.
  Кнопка «Расписать работы и запчасти» открывает в той же форме «делал сам», мастера (фильтруется по месту), строки
  работ и запчастей и гарантию; правка или копия записи, где что-то из этого есть, открывается сразу раскрытой, а
  удаление последней строки раздел не сворачивает. Стоимость — сумма строк, пока её не ввели вручную (подсказка
  «По работам и запчастям: …» и «Считать по строкам»); без строк — просто сумма. Строка запчасти: узел из
  каталога → подсказка «В прошлый раз: Mann W 712/95, 650 ₽» → одно касание заполняет бренд, артикул, цену. Встроенный каталог — около 200 узлов и работ (подвеска
  спереди и сзади, рулевое, тормоза, выхлоп, датчики, кузов, шины); детали, которые магазины продают на сторону, —
  парами «левый/правый» («Рычаг передний нижний левый», «Опора шаровая правая», «Фара левая»); общие позиции,
  которые они заменили, — «в запасе» (скрыты из подсказок, история и напоминания на них живут). Поиск — по каждому
  слову в любом порядке («рычаг перед лев»). Узла нет — «Создать «…»» заводит свой (группа «Прочее»), а набранное без выбора становится
  названием строки; пустая строка по «Готово» не закрывается молча, а просит узел или название. Бренд — комбобокс (своя история + `brands.ts`).
  Кнопка «Повторить прошлое ТО» (видна и в короткой форме) копирует строки последней записи с тем же
  названием/типом и раскрывает их.
- **Заправка:** любые два из трёх (литры, цена за литр, сумма) → третье; полный бак (по умолчанию да);
  «пропустил заправку»; АЗС — с двух букв подсказывает сети юга России и Крыма (`domain/fuelBrands.ts`: Лукойл, Роснефть,
  Газпром, Газпромнефть, Teboil, Eco Oil, Flash, Rusoil, АТАН, ТЭС, Grifon, RedPetrol… — ~27 сетей с другими написаниями:
  Shell → Teboil) и заправки города из справочника; марка топлива по умолчанию из машины.
...
Витрина `#/showcase` показывает каждый компонент во всех состояниях в обеих темах. Это инструмент разработки: маршрут
есть только при `import.meta.env.DEV`, ссылок на неё в приложении нет, в опубликованную сборку её код не попадает.
...
- Обновление (`app/appUpdate.ts`): установленное приложение, которое сворачивают и разворачивают, не
  перезагружается — поэтому новая версия проверяется при каждом возвращении в приложение (не чаще раза в минуту),
  раз в час и по «Проверить обновления» в «Настройки → О приложении»; скачанная версия — состояние, а не разовое
  событие: уведомление повторяется при возвращении, в настройках — «Обновить приложение». Там же дата сборки.
