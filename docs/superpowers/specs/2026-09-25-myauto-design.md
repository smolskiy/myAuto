...
- **ТО/ремонт:** название (подсказки «ТО-N», из прошлых), тип, место (комбобокс с созданием нового), мастер
  ...
  650 ₽» → одно касание заполняет бренд, артикул, цену. Встроенный каталог — около 150 узлов и работ (подвеска
  спереди и сзади, рулевое, тормоза, выхлоп, датчики, кузов, шины); поиск — по каждому слову в любом порядке
  («рычаг перед»). Узла нет — «Создать «…»» заводит свой (группа «Прочее»), а набранное без выбора становится
