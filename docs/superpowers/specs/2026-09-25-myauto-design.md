...
- **Выбор.** В форме машины — плитки «Чертёж на главной»: «Автоматически» (что подобралось по полям формы), все
  чертежи и «Без чертежа». Выбор хранится в `Vehicle.schematic` (id картинки или 'none'; нет поля — подбор), так
  седану без своего чертежа можно поставить Lanos, кроссоверу — X5.
