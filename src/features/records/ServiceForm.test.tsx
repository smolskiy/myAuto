...
describe('строка, узла которой нет в каталоге', () => {
  ...
  test('поиск в «Узел» и закрытие крестиком — новой строки нет, прежняя строка цела', async () => {
    ...
    let sheet = await screen.findByRole('dialog', { name: 'Работа' })
    await userEvent.type(within(sheet).getByRole('combobox', { name: 'Узел' }), 'торм')
    ...
    expect(within(works).queryByText('торм')).not.toBeInTheDocument()

    await userEvent.click(screen.getByRole('button', { name: 'Добавить работу' }))
    sheet = await screen.findByRole('dialog', { name: 'Работа' })
    await userEvent.type(within(sheet).getByRole('combobox', { name: 'Узел' }), 'мас')
    await userEvent.click(within(sheet).getByRole('button', { name: 'Закрыть' }))
    await waitFor(() => expect(screen.queryByRole('dialog', { name: 'Работа' })).not.toBeInTheDocument())
    expect(within(works).getAllByRole('listitem')).toHaveLength(1)
  })
  ...
  test('точное название узла без выбора из списка — строка привязана к узлу', async () => {
    ...
    await closeSheet(sheet)
    await userEvent.click(screen.getByRole('button', { name: 'Сохранить' }))
    await waitFor(async () => expect(await savedServices()).toHaveLength(1))
    const [saved] = await savedServices()
    expect(saved?.parts[0]).toMatchObject({ itemId: CATALOG_ID.engineOil, name: 'Моторное масло' })
  })
