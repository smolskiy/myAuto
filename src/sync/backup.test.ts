...
test('Excel: только живые строки, заправка и журнал в рублях', async () => {
  ...
  await repos.records.create({ vehicleId: v.id, kind: 'fuel', date: '2026-09-20', odometer: 145400, total: 280050,
    liters: 45, pricePerLiter: 6223, fullTank: true, missedBefore: false, fuelGrade: 'АИ-95', placeId: azs.id })
  const wash = await repos.records.create({ vehicleId: v.id, kind: 'expense', date: '2026-09-21', total: 50000, category: 'wash' })
  ...
  expect(rows('Журнал')[0]).toMatchObject({ Машина: 'Октавия', Дата: '2026-09-20', Тип: 'Заправка', Пробег: 145400, 'Сумма, ₽': 2800.5, Место: 'Лукойл' })
